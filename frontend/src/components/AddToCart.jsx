...
function AddToCart({ product, buttonText }) {
  ...
  return (
    ...
    >
      {buttonText ? (
        <button
          className={`bg-primary w-full text-background font-semibold py-1 px-6 text-md  transition delay-50 hover:bg-primaryShadow hover:ease-in-out  flex gap-3 justify-center items-center `}
          onClick={scrollToTop}
        >
          Add to Cart
        </button>
